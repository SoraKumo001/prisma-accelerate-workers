...
export class PrismaAccelerate {
	...
	static createApiKey = async ({ secret, datasourceUrl }: { secret: string; datasourceUrl: string }) => {
	...
	async query({ hash, headers, body }: { hash: string; headers: IncomingHttpHeaders; body: unknown }) {
		...
		return prisma._engine.request(query, { isWrite: true }).catch((e: { message: string; code: number; meta: unknown }) => {
			return {
				errors: [
					{
						error: String(e),
	...
	async startTransaction({ version, hash, headers, body }: { version: string; hash: string; headers: IncomingHttpHeaders; body: unknown }) {
		const prisma = await this.getPrisma({ hash, headers });
		if (!prisma) return;
	...
	async queryTransaction({ hash, headers, body, id }: { hash: string; headers: IncomingHttpHeaders; body: unknown; id: string }) {
		const prisma = await this.getPrisma({ hash, headers });
		if (!prisma) return;
		const query = JSON.parse(body as string);
		const result = await prisma._engine
	...
	async commitTransaction({ hash, headers, id }: { hash: string; headers: IncomingHttpHeaders; id: string }) {
		const prisma = await this.getPrisma({ hash, headers });
		if (!prisma) return;
		return prisma._engine.transaction('commit', {}, { id, payload: {} });
	}
	async rollbackTransaction({ hash, headers, id }: { hash: string; headers: IncomingHttpHeaders; id: string }) {
		const prisma = await this.getPrisma({ hash, headers });
		if (!prisma) return;
	...
	createPrismaClient({
		...
		const resolve = async () => {
			...
			return this.config.adapter ? new PrismaClient({ adapter: this.config.adapter(datasourceUrl) }) : new PrismaClient({ datasourceUrl });
		};
	...
	async updateSchema({ hash, headers, body }: { hash: string; headers: IncomingHttpHeaders; body: unknown }) {
